/**
 * Export utilities for reports - browser-only, always called client-side
 */
...
<head>
  ...
  <style>
    ...
    .kpi-value { font-size: 32px; font-weight: bold; color: #1e40af; margin: 10px 0; }
...
<body>
  ...
  <div class="kpi-grid">
    <div class="kpi-card"><div class="kpi-label">Total Requests</div><div class="kpi-value">${summary.totalRequests || 0}</div></div>
    <div class="kpi-card"><div class="kpi-label">Active Projects</div><div class="kpi-value">${summary.activeProjects || 0}</div></div>
    <div class="kpi-card"><div class="kpi-label">Completed</div><div class="kpi-value">${summary.completed || 0}</div></div>
    <div class="kpi-card"><div class="kpi-label">Delayed</div><div class="kpi-value">${summary.delayed || 0}</div></div>
    <div class="kpi-card"><div class="kpi-label">Avg Cycle Time</div><div class="kpi-value">${summary.avgCycleTimeDays || 0} days</div></div>
    <div class="kpi-card"><div class="kpi-label">SLA Achievement</div><div class="kpi-value">${summary.slaAchievement || 0}%</div></div>
  </div>
  <h2>Requests by Status</h2>
  <table>
    <thead><tr><th>Status</th><th>Count</th><th>Percentage</th></tr></thead>
    <tbody>${statusData.map((item: any) => `<tr><td>${item.status}</td><td>${item.count}</td><td>${item.percentage}%</td></tr>`).join('')}</tbody>
  </table>
  <h2>Requests by Priority</h2>
  <table>
    <thead><tr><th>Priority</th><th>Count</th><th>Percentage</th></tr></thead>
    <tbody>${priorityData.map((item: any) => `<tr><td style="text-transform:capitalize">${item.priority}</td><td>${item.count}</td><td>${item.percentage}%</td></tr>`).join('')}</tbody>
  </table>
